...
class MPNN(nn.Module):
    ...
    def forward(self, obs):
        ...
        # print('HIDDEN NODE FEATURES', init_node_embeddings.shape)
        edge_embeddings = self.edge_embedding_layer(node_features, adj, norm)
        # print('FINAL EDGE EMB', edge_embeddings.shape)
        ...
        if self.tied_weights:
            ...
            for i in range(self.n_layers):
                current_node_embeddings = self.update_node_embedding_layer[i](current_node_embeddings,
                                                                              ...
                                                                              norm,
                                                                              adj)
        # print('FINAL NODE EMB', current_node_embeddings.shape)
        out = self.readout_layer(current_node_embeddings)
        # print('out', out.squeeze().shape)
        out = out.squeeze()
...
class EdgeAndNodeEmbeddingLayer(nn.Module):
    ...
    def forward(self, node_features, adj, norm):
        ...
        # print('norm:', norm)
        # print(norm.shape)
        # print(node_features.unsqueeze(-2).shape)
        # print('node_features.unsqueeze(-2):', node_features.unsqueeze(-2))
        # print(node_features.unsqueeze(-2).transpose(-2, -3).shape)
        # print('node_features.unsqueeze(-2).transpose(-2, -3):', node_features.unsqueeze(-2).transpose(-2, -3))
        # print(node_features.unsqueeze(-2).transpose(-2, -3).repeat(1, adj.shape[-2], 1, 1).shape)
        # print('node_features.unsqueeze(-2).transpose(-2, -3).repeat(1, adj.shape[-2], 1, 1):', node_features.unsqueeze(-2).transpose(-2, -3).repeat(1, adj.shape[-2], 1, 1))
        # print(adj.unsqueeze(-1).shape)
        # print('adj.unsqueeze(-1):', adj.unsqueeze(-1))
        ...
        # print('(adj.unsqueeze(-1)!=0).float():', (adj.unsqueeze(-1)!=0).float())
